...
inline int charAtRadix(const string& s, int d, long long& /*cmpCount*/) {
    // Radix sort не делает посимвольных сравнений между строками,
    // он только читает символ для классификации в корзину.
    // По условию задания такие операции не учитываются.
    if (d < (int)s.size()) {
